...
class MemoryBank(nn.Module):
    ...
    def forward(self, inputs, inputs_s, inputs_s1, indexes, k=10):
        ...
        print('target_label', target_label.device)
        print('target_idx', type(target_idx))
        for i in target_idx:
        ...
        print('torch.ones(self.num_samples,1)', torch.ones(self.num_samples,1).device)
        # print('torch.ones(self.num_samples,1)[target_idx].float().cuda()',
        #       torch.ones(self.num_samples,1)[target_idx].float().cuda().device)
        target_idx = tuple([_i.cpu() for _i in target_idx])
        t_nums.index_add_(0, target_label.cuda(), torch.ones(self.num_samples,1)[target_idx].float().cuda())
        ...
        source_idx = tuple([_i.cpu() for _i in source_idx])
        s_nums.index_add_(0, source_label.cuda(), torch.ones(self.num_samples,1).cuda()[source_idx].float().cuda())
